...
```{r auc}
obs = test$DRK_YN #valor verdadero
pred = predict(tree,test,type="prob")[, "1"]
...
# 70% NAs en data
data_70pc_na <- data
data_70pc_na[,-which(names(data) == "DRK_YN")] <- lapply(data_70pc_na[,-which(names(data) == "DRK_YN")], asignar_na, porcentaje = 70)

#Creo los conjuntos de train, validacion y test con 70% NAs
nobs <- nrow(data_70pc_na)
itrain <- sample(nobs, 0.7*nobs)
train_70 <- data_70pc_na[itrain, ]
resto <- data_70pc_na[-itrain,]
nobs2 <- nrow(resto) 
ival <- sample(nobs2, 0.5*nobs2)
val_70 <- resto[ival,]
test_70 <- resto[-ival,]
...
```{r tree 20%}
tree <- rpart(formula=DRK_YN ~., data = train_20, method="class")
```


```{r tree 50%}
tree <- rpart(formula=DRK_YN ~., data = train_50, method="class")
```


```{r tree 70%}
tree <- rpart(formula=DRK_YN ~., data = train_70, method="class")
```
